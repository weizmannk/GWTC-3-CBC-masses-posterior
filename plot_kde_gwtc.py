        ...
        if gw_name in bns_gw_names:
            if gw_name == "GW190814":
                ax = sns.kdeplot(
                    ...
                    x="mass1",
                    y="mass2",
                    color=GW190814_color,
                    levels=1,
                    thresh=0.1,
                )
            else:
                ax = sns.kdeplot(
                    data=df, x="mass1", y="mass2", color=bns_color, levels=1, thresh=0.1
                )

        elif gw_name in nsbh_gw_names:
            ax = sns.kdeplot(
                data=df, x="mass1", y="mass2", color=nsbh_color, levels=1, thresh=0.1
            )

        else:
            if (
                gw_name in bbh_gw_names
                and gw_name not in bns_gw_names + nsbh_gw_names + ["GW190725"]
            ):
                ax = sns.kdeplot(
                    data=df, x="mass1", y="mass2", color=bbh_color, levels=1, thresh=0.1
                )
